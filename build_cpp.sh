...
catkin_make $VERBOSE \
       --cmake-args -DCMAKE_TOOLCHAIN_FILE=$RBA_TOOLCHAIN \
       ...
       -DPYTHON_INCLUDE_DIR2=$python2_inc \
       -DBUILD_SHARED_LIBS=0 \
       -DCMAKE_INSTALL_PREFIX=$CMAKE_PREFIX_PATH \
       -DBoost_NO_BOOST_CMAKE=ON \
       -DBOOST_ROOT=$CMAKE_PREFIX_PATH \
       -DANDROID=TRUE \
       -DBOOST_INCLUDEDIR=$CMAKE_PREFIX_PATH/include/boost \
       -DBOOST_LIBRARYDIR=$CMAKE_PREFIX_PATH/lib \
       -DROSCONSOLE_BACKEND=print \
       -DCMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE} \
       -DANDROID_ABI=$ANDROID_ABI \
       -DCMAKE_FIND_ROOT_PATH=$prefix \
       -DOpenCV_LIB_PATH=$prefix/libs/opencv-2.4.11/build
